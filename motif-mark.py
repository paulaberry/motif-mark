...
# color palettes
palettes = {'viridis': [(0.267004, 0.004874, 0.329415, 1.0), (0.275191, 0.194905, 0.496005, 1.0), (0.214298, 0.355619, 0.551184, 1.0), (0.154815, 0.493313, 0.55784, 1.0), (0.120638, 0.625828, 0.533488, 1.0), (0.274149, 0.751988, 0.436601, 1.0), (0.595839, 0.848717, 0.243329, 1.0), (0.964894, 0.902323, 0.123941, 1.0)],
...
'cvd_safe': [(0.0, 0.4666666666666667, 0.7333333333333333, 1), (0.2, 0.7333333333333333, 0.9333333333333333, 1), (0.0, 0.6, 0.5333333333333333, 1), (0.9333333333333333, 0.4666666666666667, 0.2, 1), (0.8, 0.2, 0.06666666666666667, 1), (0.9333333333333333, 0.2, 0.4666666666666667, 1), (0.5333333333333333, 0.13333333333333333, 0.3333333333333333, 1), (0.6666666666666666, 0.26666666666666666, 0.6, 1)]}

# Set up color palette and options
custom = False
...
if len(colormap) == 1: # check if colormap is valid
    ...
    if mapstring.find(colormap[0]) != -1:
        ...
        color_list = []
        for i in colormap:
            if i not in custom_colors: # check if a custom color is valid
                print(str(i) + " is not a valid color selection!")
            else:
                color_list.append(custom_colors[i])
                custom = True
        colormap = color_list
else: # check if a valid custom color
    color_list = []
    for i in colormap:
        if i not in custom_colors:
            print(str(i) + " is not a valid color selection!")
        else:
            color_list.append(custom_colors[i])
            custom = True
...
for key in motif_dictionary:
    motif_dictionary[key].append(colormap[color_n])
